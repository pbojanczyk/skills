...
Hippocortex automatically detects and encrypts secrets (API keys, passwords, tokens) found in conversations. Users can also manually add secrets via the Dashboard. Every tenant gets a **Main Vault** automatically.

### Search the vault

Find secrets by natural language query. Returns metadata only (titles, types, tags) — never the actual values.

```bash
curl -s -X POST "$BASE_URL/v1/vault/query" \
  -H "Authorization: Bearer $HIPPOCORTEX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "query": "my OpenAI API key",
    "limit": 5
  }'
```

Returns: `{ "results": [{ "id": "...", "title": "...", "type": "api_key", "tags": [...], "relevance": 0.95 }] }`

### Reveal a secret

Retrieve the actual decrypted value. Requires reveal permission. Rate limited (10/min). All access is logged.

```bash
curl -s -X POST "$BASE_URL/v1/vault/query/<itemId>/reveal" \
  -H "Authorization: Bearer $HIPPOCORTEX_API_KEY" \
  -H "Content-Type: application/json"
```

Returns: `{ "value": "sk-...", "title": "OpenAI API Key" }`

### Synthesize with vault resolution

When retrieving context, vault references (`[vault:item_id]`) in memories can be resolved inline:
...
curl -s -X POST "$BASE_URL/v1/synthesize" \
  ...
  -d '{
    "sessionId": "'"$SESSION_ID"'",
    "query": "what API keys do I have?",
    "resolveVaultRefs": true,
    "maxTokens": 2000
  }'
```

**Security:** `resolveVaultRefs` defaults to `false`. Only enable when the agent explicitly needs secret values. All vault access is audited.

### Vault in the Dashboard

- **Main Vault** is auto-created for every account — no setup needed
- Users can manually add secrets (name, value, tags)
- Auto-captured secrets are tagged automatically
- Cross-references show which memories link to which secrets
