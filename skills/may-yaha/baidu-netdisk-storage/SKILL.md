...
自动检测平台架构，下载对应的 bdpan CLI 安装器并执行安装。完整性校验由安装器内部完成。

**安全说明：**
- 安装器从百度 CDN（`issuecdn.baidupcs.com`）下载
- SHA256 完整性校验由 bdpan CLI 安装器内部处理

> **安装机制风险声明：** 安装和更新过程涉及从百度官方端点（`issuecdn.baidupcs.com`、`pan.baidu.com`）下载二进制文件/压缩包并在本地执行，属于 download+execute 模式。已通过以下措施缓解风险：安装器内部 SHA256 校验、仅使用百度官方域名、更新需用户明确发起并确认。建议用户在首次安装或安全敏感场景下，通过沙箱环境执行安装或手动审查下载的二进制文件。
