...
import { apiRequest, type HttpClientConfig } from './http-client.js';
import { getQueryType, type BqFilter } from './bq-builder.js';
...
const EDIT_TYPE_MAP: Record<string, boolean> = {
  text: true,
  multiline_text: true,
  date: true,
  datetime: true,
...
const VALUE_DESC_MAP: Record<string, string> = {
  text: '填写字符串',
...
function isEditable(field: any): boolean {
  const type = field.type as string;
  if (type === 'reference') return field.dependOn === field.name;
  return EDIT_TYPE_MAP[type] ?? false;
}

function getValueDesc(field: any): string {
  const type = field.type as string;
...
function extractOptions(field: any): OptionItem[] | undefined {
  const type = field.type as string;
  if (type !== 'dropdown_list' && type !== 'checkbox' && type !== 'tree') return undefined;
...
export async function fetchFieldDefs(
  ...
  spaceId: string,
  formId: string
): Promise<{ success: boolean; message: string; fields: FieldDef[] }> {
  ...
  const fields: FieldDef[] = rawFields.map((f: any) => {
    ...
    if (f.maxlen) def.maxlen = f.maxlen;
    if (f.maxvalue) def.maxvalue = f.maxvalue;
    if (f.minvalue) def.minvalue = f.minvalue;
    if (f.pointnum) def.pointnum = f.pointnum;
    if (f.unique === 'true') def.unique = true;
    if (f.dependOn) def.dependOn = f.dependOn;
    if (f.sourceFormId) def.sourceFormId = f.sourceFormId;
    if (f.sourceFieldName) def.sourceFieldName = f.sourceFieldName;
    return def;
  });
 ...
 */
export function buildFieldMaps(fields: FieldDef[]) {
 ...
 */
export function mapFilterFieldNames(
  fields: FieldDef[],
  filters: BqFilter[]
): { mapped: BqFilter[]; warnings: string[] } {
  const { labelToName, nameSet, nameToLabel } = buildFieldMaps(fields);
  const mapped: BqFilter[] = [];
  const warnings: string[] = [];
 ...
 */
export function buildMappingSummary(fields: FieldDef[]): string {
  return fields
    .filter((f) => f.editable)
    .map((f) => {
