...
openclaw agent --session-id test_cuj_1 --message "无需检查文件内容，请立刻根据 leio-sdlc 的 Command Template 1 规范，调用 exec 工具执行: SDLC_TEST_MODE=true python3 /root/.openclaw/skills/leio-sdlc/scripts/spawn_planner.py --prd-file /root/.openclaw/workspace/docs/PRDs/dummy.md"
