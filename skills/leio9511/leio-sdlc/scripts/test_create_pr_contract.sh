...
TEST_DIR="/tmp/test_queue_dir"
rm -rf "$TEST_DIR"
mkdir -p "$TEST_DIR"

CONTENT_FILE="/tmp/test_content.txt"
echo "Test PR content" > "$CONTENT_FILE"

SCRIPT_PATH="$(dirname "$0")/create_pr_contract.py"

# Negative: Missing content file
if python3 "$SCRIPT_PATH" --job-dir "$TEST_DIR" --title "Fail" --content-file "/tmp/non_existent.txt" 2>/dev/null; then
  echo "❌ Expected failure for missing content file, but it succeeded."
  exit 1
fi

# Positive 1: First PR
OUTPUT1=$(python3 "$SCRIPT_PATH" --job-dir "$TEST_DIR" --title "First PR" --content-file "$CONTENT_FILE")
...
head -n 2 "$FILE1" | grep -q "status: open" || { echo "❌ Missing status injection"; exit 1; }

# Positive 2: Second PR
OUTPUT2=$(python3 "$SCRIPT_PATH" --job-dir "$TEST_DIR" --title "Second PR" --content-file "$CONTENT_FILE")
...
OUTPUT3=$(python3 "$SCRIPT_PATH" --job-dir "$TEST_DIR" --title "Insert One" --content-file "$CONTENT_FILE" --insert-after "001")
...
OUTPUT4=$(python3 "$SCRIPT_PATH" --job-dir "$TEST_DIR" --title "Insert Two" --content-file "$CONTENT_FILE" --insert-after "001")
...
OUTPUT5=$(python3 "$SCRIPT_PATH" --job-dir "$TEST_DIR" --title "Third PR" --content-file "$CONTENT_FILE")
