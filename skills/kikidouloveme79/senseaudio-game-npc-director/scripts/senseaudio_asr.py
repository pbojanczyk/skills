...
def transcribe_openapi(path: Path, api_key: str, model: str, response_format: str, language: str, stream: bool) -> dict:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Codex-SenseAudio-Game-ASR/1.0",
        },
        method="POST",
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Transcribe player audio with SenseAudio official ASR endpoints.")
    ...
    mode = args.mode
    if mode == "auto":
        mode = "platform" if platform_token else "openapi"
