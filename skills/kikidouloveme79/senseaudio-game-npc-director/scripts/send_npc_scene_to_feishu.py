...
feishu_sender = load_helper_module("audioclaw-skills-voice-reply", "feishu_audio_sender.py", "npc_feishu_sender")
...
def slugify(value: str) -> str:
    ...
    return cleaned.strip("_") or "npc_scene"
...
def filter_results(results: list[dict], line_ids: str, limit: int) -> list[dict]:
    selected = results
    if line_ids.strip():
        wanted = {item.strip() for item in line_ids.split(",") if item.strip()}
        selected = [item for item in selected if item.get("line_id") in wanted]
    if limit > 0:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Send synthesized NPC scene lines to Feishu one by one as audio messages.")
    parser.add_argument("--scene-tts-results", required=True)
    ...
    parser.add_argument("--line-ids", default="")
    ...
    summary_path = Path(args.scene_tts_results).expanduser().resolve()
    summary = load_json(summary_path)
    selected_results = filter_results(summary.get("results") or [], args.line_ids, args.limit)
    if not selected_results:
        raise SystemExit("no NPC lines selected for Feishu sending")
    ...
    if args.outdir.strip():
        ...
        npc_slug = slugify(summary.get("npc_name") or "npc")
        prepared_dir = workspace_root / "state" / "audio" / "npc_scene" / npc_slug
    ...
    for index, item in enumerate(selected_results, start=1):
        ...
        if not source_path.exists():
            raise SystemExit(f"missing synthesized NPC audio file: {source_path}")
        target_name = f"{item.get('line_id', f'L{index:02d}')}_{slugify(str(summary.get('npc_name') or 'npc'))}.ogg"
        target_path = prepared_dir / target_name
        transcode_to_ogg(source_path, target_path, ffmpeg_exe=ffmpeg_exe)

        if args.send_labels:
            label = f"{summary.get('npc_name', 'NPC')} {item.get('line_id', f'L{index:02d}')}"
    ...
    result = {
        "mode": "npc_scene_feishu_sequence_sent",
        "npc_name": summary.get("npc_name"),
        "chat_id": chat_id,
