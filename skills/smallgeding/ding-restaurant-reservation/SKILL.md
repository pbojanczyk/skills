...
description: |
DINGs 托管式 AI 电话助手 - 中国、日本餐厅全自动 AI 电话预订技能，支持餐厅搜索+AI 电话自动预订一体化服务，只要你告诉我用餐需求（或需搜索的餐厅信息），即可帮你完成餐厅搜索、自动拨打餐厅电话完成预订的全流程任务。
当用户提到以下意图时使用此技能:
...
支持:中国、日本餐厅搜索、AI 电话自动外呼预订、预订结果回调通知、预订状态查询、通话记录查看、电子凭证生成。
...
DINGs 托管式 AI 电话助手(中国、日本餐厅专版)API
⚠️ 必读约束
...
传入名称/城市/国家code，返回餐厅列表（含dings_id）；城市需按支持列表填写
...
方法2：搜索餐厅后预订
1. 助手询问用户需搜索的餐厅名称、所在城市（按支持列表）、所属国家（中国/日本）；
...
接口说明：根据餐厅名称、城市（按支持列表填写）、国家code搜索中国/日本餐厅，返回餐厅列表（含dings_id、可预订状态等核心信息），为后续预订提供餐厅数据支撑。
...
城市入参参考&支持列表
接口city_name入参需严格匹配下方列表中的城市名称，中国城市使用中文全称，日本城市支持中文/日文名称（如东京/とうきょう）。
中国（CN）支持城市列表
...
400
请求参数错误
检查必填参数(如order_id、restaurant_name不能为空)；检查city_name是否在支持列表内；确认请求方法为POST
404
资源未找到
确认订单ID/餐厅信息是否正确，订单/餐厅是否存在；确认城市+国家code匹配是否正确
500
服务器内部错误
稍后重试，或联系平台客服
...
技能版本:v1.3.0 | 最后更新:2026-03-27


---

核心修改总结
1. 接口方法：餐厅搜索接口 GET → POST，全文档同步更新
2. 请求示例：替换为官方POST格式curl命令
3. 校验规则：新增请求方法错误错误码，最佳实践新增POST调用规范
4. 版本升级：技能版本更新为 v1.3.0，更新时间同步修改
5. 参数描述：按照你提供的最新格式优化city_name描述