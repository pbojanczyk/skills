...
[![Encryption: Optional](https://img.shields.io/badge/Encryption-AES--256--GCM-orange.svg)](#数据加密)
...
- Python 3.10+
- 核心功能零第三方依赖
- 仅开启加密时需要 `cryptography` 包（`pip install cryptography`）
...
| 🔐 **AES-256-GCM 加密** | 强烈建议开启 ---- 加密所有敏感数据文件 |
...
- **无后门**：密码丢失 = 数据丢失
- **密码输入方式**：交互式输入（推荐）、`SOUL_PASSWORD` 环境变量（请确保运行环境安全）、`--password` 参数（不推荐，会留在 shell history）
- **依赖**：`pip install cryptography`（仅加密功能需要，非核心依赖）
- **环境变量**：`SOUL_PASSWORD` -- 仅加密模式下需要
...
| 层面 | 技术 |
|------|------|
| 核心语言 | Python 3（纯标准库，零依赖；加密功能需 `cryptography` 包，非核心） |
