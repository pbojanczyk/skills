...
env:
  SOUL_PASSWORD:
    description: "AES-256-GCM encryption password for soul data (optional, recommended for privacy)"
    required: false
requirements:
  - "Python 3.10+"
...
灵魂存档是一个**数字人格持久化系统**。它在用户授权或显式触发的前提下，提取并归档用户的：
...
- **数据流声明**：soul_chat 模式会将存档数据构建为 prompt 供 agent 使用；这些 prompt 是否发送到外部 LLM 取决于 agent/平台配置，与本 skill 无关。如对此有顾虑，请确保 agent 使用本地模型或可信的 LLM 端点
- **支持 AES-256-GCM 加密**（默认开启），灵魂存档包含高度隐私的个人信息（身份、性格、语言指纹、情感模式、人际关系），加密后即使文件被复制，没有密码也无法读取
...
### 🤫 非侵入式采集
- 不打断对话流，不额外追问
- 通过触发词显式激活，或在 opt-in 模式下自动运行
- 只在发现**新的、有价值的**信息时才更新存档

> ⚠️ **知情权提醒**：自动提取意味着 AI 在对话过程中会自动提取人格信息。如需完全控制，可将 `config.json` 中的 `auto_extract` 设为 `false`，改为手动触发（说"沉淀一下"）。首次使用前建议先审查 `config.json` 配置。
...
python3 scripts/soul_report.py \
  --output ~/WorkBuddy/Claw/soul-report.html
```
...
**触发**：用户说以下任意触发词，或 AI 完成实质性任务后自动触发（受 `config.json` 中的 `auto_reflect` 控制，默认开启）。
...
> 数据默认存储在 `~/.skills_data/soul-archive/`。
...
**可通过 `config.json` 控制开关**（`auto_extract` 和 `auto_reflect`），默认**关闭**自动提取，但**开启**自我反思。

当两者均为 `true` 时，**每次与用户的实质性对话结束后**，AI 自动执行以下两项：

**1. 灵魂沉淀（用户维度）** -- 受 `auto_extract` 控制：
1. 回顾本次对话内容
2. 识别是否包含新的人格信息
3. 如果有，调用 `soul_extract.py` 进行提取
4. 更新存档，不打断用户

**2. 自我反思（AI 维度）** -- 受 `auto_reflect` 控制：
1. 回顾本次工作过程
2. 识别做得好和做得不好的地方
...
> ⚠️ **知情权**：默认配置 `auto_extract=false`，自动提取**默认关闭**，需用户主动开启。
> ⚠️ **自我反思默认开启**（`auto_reflect=true`），可随时在 `config.json` 中关闭。
> 这是指令级约束：加载了本 SKILL.md 的任何 Agent 都必须遵守此规则，不依赖 hooks 机制。
...
| 语言风格 | 25% | 口头禅:log(120万), 句式:log(100万), 例句:log(400万) | 列表曲线+枚举线性(仅2%) |
...
灵魂存档支持 **AES-256-GCM** 加密保护所有敏感数据。

> ⚠️ **强烈建议开启加密**。灵魂存档包含高度隐私的个人信息（身份、性格、语言指纹、情感模式、人际关系），一旦数据文件泄露，后果不可挽回。开启 AES-256-GCM 加密后，即使文件被复制，没有密码也无法读取。
...
> ⚠️ **不推荐用环境变量存密码** -- 环境变量可能被其他进程读取。如非自动化必需，建议用交互式输入。

优先级从高到低：
1. `--password` 参数（不推荐，会留在 shell history）
2. `SOUL_PASSWORD` 环境变量（适合自动化/AI 调用，**请确保运行环境安全**）
3. 交互式输入（推荐，不回显）
