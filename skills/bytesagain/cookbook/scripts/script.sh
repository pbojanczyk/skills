#!/usr/bin/env bash
# cookbook -- Track cookbook personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COOKBOOK_DIR:-$HOME/.cookbook}"
...
show_help() {
    cat << EOF
cookbook v$VERSION -- Track cookbook personal activities and daily routines. Use when managing health goals, organizing tasks, or building productive habits.

Usage: cookbook <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cookbook add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cookbook search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cookbook remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cookbook v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cookbook help"; exit 1 ;;
esac
