#!/usr/bin/env bash
# dcf — Dcf reference tool. Use when working with dcf in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
dcf v$VERSION — Dcf Reference Tool
...
## What is Dcf?
Dcf (dcf) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with dcf.

## Key Concepts
- Core dcf principles and fundamentals
- How dcf fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Dcf Matters
Understanding dcf is critical for:
...
## Getting Started
1. Understand the basic dcf concepts
...
cmd_instruments() {
    cat << 'EOF'
# Dcf — Instruments & Tools Overview

## Primary Instruments
- Core tools used in dcf operations
...
cmd_glossary() {
    cat << 'EOF'
# Dcf — Key Terms & Definitions

## Core Terminology
- **Dcf**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "dcf v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: dcf help"; exit 1 ;;
esac
