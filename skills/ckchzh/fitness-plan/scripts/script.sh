#!/usr/bin/env bash
# fitness-plan - Productivity and task management tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${FITNESS_PLAN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/fitness-plan}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
fitness-plan v$VERSION

Productivity and task management tool

Usage: fitness-plan <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "fitness-plan v$VERSION" ;;
